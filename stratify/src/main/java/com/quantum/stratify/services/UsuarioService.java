...
public class UsuarioService {
    ...
    public Usuario salvar(Usuario usuario) {
        try {
            if (usuario.getSenha() == null || usuario.getSenha().isBlank()) {
                throw new PasswordInvalidException("Senha não pode ser nula ou vazia");
            }
            ...
            throw new UsernameUniqueViolationException(
                String.format("Email '%s' já cadastrado", usuario.getEmail()));
        }
    }
    ...
    public Usuario resetarSenha(Long idUsuario, String novaSenha) {
        ...
        if (novaSenha == null || novaSenha.isBlank()) {
            throw new PasswordInvalidException("Nova senha não pode ser nula ou vazia");
        }
    ...
    public Usuario buscarPorEmail(String email) {
        return usuarioRepository.findByEmail(email)
                .orElseThrow(() -> new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Usuário com email '" + email + "' não encontrado"));
    }
    ...
    public void atribuirLideradosAoGestor(AtribuirGestor dto) {
        ...
        dto.getListaIdLiderados().forEach(idLiderado -> {
            Usuario liderado = usuarioRepository.findById(idLiderado)
                    .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Usuário liderado não encontrado: " + idLiderado));
    ...
    public List<UsuarioInfoDTO> listarUsuariosInfo() {
        return usuarioRepository.findAll().stream()
                .map(u -> new UsuarioInfoDTO(
                        ...
                        u.getNome(),
                        u.getEmail(),
                        u.getGestor() != null ? u.getGestor().getNome() : null,
                        u.getRole(),
                        u.isRequireReset(),
                        u.getIsEnable()))
                .toList();
